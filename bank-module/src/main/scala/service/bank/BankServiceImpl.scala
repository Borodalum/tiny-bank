...
private final class BankServiceImpl[F[_] : Monad](
  ...
  override def removeAcc(id: BankAccountId): F[Either[ApiError, Unit]] =
    bankRepository
      .removeById(id)
      .leftMapIn(fromDbError)
  ...
  override def withdrawFromAcc(id: BankAccountId, amount: Money): F[Either[ApiError, Unit]] =
    bankRepository
      .getBalanceById(id)
      .flatMap {
  ...
  override def transfer(fromId: BankAccountId, accTransfer: BankAccountTransfer): F[Either[ApiError, Unit]] =
    bankRepository
  ...
  override def getBalance(id: BankAccountId): F[Either[ApiError, BankAccountBalance]] =
    bankRepository
      .getBalanceById(id)
      .flatMap {
